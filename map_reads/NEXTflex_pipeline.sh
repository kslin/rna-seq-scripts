...
if [ "$#" -ne 5 ]; then
    ...
    printf "Usage: `basename $0` <fastq file> <STAR index> <exon annotation file> <intron annotation file> <output directory>";
    exit
fi
...
if [ ! -f "$exon_file" ] ;
then
   echo -e "Cannot find exon annotation file $exon_file";
   exit
fi
if [ ! -f "$intron_file" ] ;
then
...
# samtools sort -O "sam" -o "$out_dir/${filename}_revcomp_star_sorted.sam" -T "$out_dir/${filename}_TEMP" -@ "24" "$out_dir/${filename}_revcomp_star_Aligned.out.sam" &&
...
intersectBed -bed -wo -s -abam "$out_dir/${filename}_revcomp_star_sorted.bam" -b "$exon_file" > "$out_dir/${filename}_revcomp_star_sorted_exons.bed"; intersectBed -bed -wo -s -abam "$out_dir/${filename}_revcomp_star_sorted.bam" -b "$intron_file" > "$out_dir/${filename}_revcomp_star_sorted_introns.bed" &&

# compile counts
cut -f "16" "$out_dir/${filename}_revcomp_star_sorted_exons.bed" | "sort" | "uniq" -c > "$out_dir/${filename}_revcomp_star_sorted_exon_counts.txt" &&
